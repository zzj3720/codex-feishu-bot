...
部署好的机器人现在支持一个内置快捷口令，用来让 Codex 代你跑 `lark-op-cli` 的建机器人流程。
...
机器人会自动把这条消息展开成一段预置提示词，让 Codex：

- 异步执行 `npx -y lark-op-cli@latest create-bot`
- 持续读取 stdout / stderr
- 如果出现扫码登录，把 ASCII 二维码原样转发回飞书
- 在命令完成后总结创建结果和后续动作

这个快捷口令只会触发建机器人流程，不会默认继续部署当前仓库；除非你在后续消息里明确要求它继续。
