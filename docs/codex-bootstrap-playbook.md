...
- If the user's prompt explicitly says not to open a browser and to only run `npx -y lark-op-cli@latest create-bot`, treat that as a narrower flow.
- In that narrower flow, execute only `create-bot`, stream output, surface any ASCII QR login prompt, then stop and summarize the result.
